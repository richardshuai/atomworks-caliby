...
def test_remove_crystallization_aids(pdbid: str):
    ...
    out1 = CIF_PARSER_BIOTITE.parse(filename=get_pdb_path(pdbid), remove_ccds=[])
    assert np.any(
    ...
    out2 = CIF_PARSER_BIOTITE.parse(filename=get_pdb_path(pdbid), remove_ccds=CRYSTALLIZATION_AIDS)
    assert not np.any(
