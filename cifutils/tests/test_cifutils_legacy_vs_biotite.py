...
def validate_chains(pdb_id, chains_legacy, converted_chains):
    for chain_id, converted_chain in converted_chains.items():
        ...
        assert len(legacy_atom_ids) == len(converted_atom_ids), \
        ...
        assert legacy_atom_ids == converted_atom_ids, \
        ...
        for atom_id in legacy_atom_ids:
            ...
            assert legacy_atom.leaving == converted_atom.leaving, f"Leaving atom flag mismatch for atom {atom_id} within chain {chain_id} in PDB ID {pdb_id}"
            assert sorted(legacy_atom.leaving_group) == sorted(converted_atom.leaving_group), f"Leaving group mismatch for atom {atom_id} within chain {chain_id} in PDB ID {pdb_id}"
            assert legacy_atom.metal == converted_atom.metal, f"Metal flag mismatch for atom {atom_id} within chain {chain_id} in PDB ID {pdb_id}"
            assert legacy_atom.hyb == converted_atom.hyb, f"Hybridization mismatch for atom {atom_id} within chain {chain_id} in PDB ID {pdb_id}"
            assert legacy_atom.nhyd == converted_atom.nhyd, f"Number of hydrogens mismatch for atom {atom_id} within chain {chain_id} in PDB ID {pdb_id}"
        ...
        legacy_bond_dict = {
            tuple(sorted((bond.a, bond.b))): bond for bond in legacy_bonds
        }
        converted_bond_dict = {
            tuple(sorted((bond.a, bond.b))): bond for bond in converted_bonds
        }

        # Assert number of bonds
        assert len(set(legacy_bond_dict.keys())) == len(set(converted_bond_dict.keys())), f"Number of bonds mismatch for chain {chain_id} within PDB ID: {pdb_id}"

        legacy_bond_set = set(legacy_bond_dict.keys())
        converted_bond_set = set(converted_bond_dict.keys())
        # Assert bond IDs match
        assert legacy_bond_set == converted_bond_set, \
        ...
        for bond_id in legacy_bond_set:
            ...
            assert legacy_bond.order == converted_bond.order, f"Bond order mismatch for bond {bond_id} within chain {chain_id} in PDB ID {pdb_id}"
            assert legacy_bond.aromatic == converted_bond.aromatic, f"Aromatic flag mismatch for bond {bond_id} within chain {chain_id} in PDB ID {pdb_id}"
            assert legacy_bond.intra == converted_bond.intra, f"Intra-residue flag mismatch for bond {bond_id} within chain {chain_id} in PDB ID {pdb_id}"
        ...
        legacy_chain_automorphisms = sorted([sorted(map(tuple, inner_list)) for inner_list in legacy_chain.automorphisms])
        converted_chain_automorphisms = sorted([sorted(map(tuple, inner_list)) for inner_list in converted_chain.automorphisms])
...
def test_unmatched_atom_types():
    ...
    # Parse with cifutils_biotite
    result_dict = parse_with_cifutils_biotite(filename, cifutils_biotite.CIFParser(add_bonds=True, add_missing_atoms=True, build_assembly=True))
