...
class CIFParser:
    ...
    def parse(self, filename):
        ...
        chain_info_dict, residue_info_dict = self._load_monomer_sequence_information(cif_block, chain_info_dict, atom_array)
    ...
    def _build_assembly(self, cif_block, atom_array, assembly_id=None):
        ...
        for id, op_expr, asym_id_expr in zip(
            ...
            if id == assembly_id:
                ...
                sub_structure = atom_array[
                    ..., np.isin(atom_array.chain_id, asym_ids)
                ]
                for operation in operations:
                    sub_assembly = apply_transformations(
                        sub_structure, transformations, operation
                    )
    ...
    def _add_missing_atoms(self, atom_array, chain_info_dict):
        ...
        residue_ids = []
        chain_ids = []
        for chain_id in deduplicate_iterator(struc.get_chains(atom_array)): # NOTE: We need set() since biotite considers a decrease in sequence_id as a new chain (see `5xnl`)
            ...
            for residue_index_sequential, residue_name in enumerate(residue_name_list, start=1):
                ...
                if chain_info_dict[chain_id]['is_polymer']:
                    ...
                    residue_id_nonpoly = chain_info_dict[chain_id]['residue_id_list'][residue_index_sequential - 1] # Recall that residue_index_sequential is 1-indexed
        ...
        if not np.array_equal(full_atom_array[full_atom_array_match_mask].atom_name, atom_array[present_atom_array_match_mask].atom_name):
        ...
        unmatched_heavy_atoms_mask = ~present_atom_array_match_mask & ((atom_array.element != 'H') & (atom_array.element != 'D'))
    ...
    def _get_inter_and_intra_residue_bonds(self, atom_array, chain_id, chain_type):
        ...
        for i in range(len(residues)):
            ...
            residue_name = current_res.res_name[0] # current_res.res_name is a list of identical values, so we just take the first one
            if residue_name in self.data_by_residue.index and len(self.data_by_residue.loc[residue_name]['intra_residue_bonds']) > 0:
        ...
        if atom_a_intra_residue_indices and atom_b_intra_residue_indices and intra_residue_bond_types:
            intra_residue_bonds = np.column_stack((np.concatenate(atom_a_intra_residue_indices), np.concatenate(atom_b_intra_residue_indices), np.concatenate(intra_residue_bond_types)))
        else:
            intra_residue_bonds = np.array([], dtype=np.int32).reshape(0, 3)

        leaving_atom_indices = np.concatenate(leaving_atom_indices) if leaving_atom_indices else np.array([], dtype=np.int32)
    ...
    def _add_bonds_from_struct_conn(self, cif_block, chain_info_dict, atom_array):
        ...
        struct_conn_df = struct_conn_df[struct_conn_df['conn_type_id'] == 'covale'] # Only consider covalent bonds (throw out disulfide bods, metal coordination covalent bonds, hydrogen bonds)
        struct_conn_bonds = []
        leaving_atom_indices = []
        ...
        if not struct_conn_df.empty:
            for _, row in struct_conn_df.iterrows():
                ...
                a_seq_id = row['ptnr1_label_seq_id'] if chain_info_dict[a_chain_id]['is_polymer'] else row['ptnr1_auth_seq_id']
                b_seq_id = row['ptnr2_label_seq_id'] if chain_info_dict[b_chain_id]['is_polymer'] else row['ptnr2_auth_seq_id']
    ...
    def _add_bonds(self, cif_block, atom_array, chain_info_dict):
        ...
        leaving_atom_indices = []
        struct_conn_bonds, struct_conn_leaving_atom_indices = self._add_bonds_from_struct_conn(cif_block, chain_info_dict, atom_array)
        ...
        for chain_id in deduplicate_iterator(struc.get_chains(atom_array)):
            chain_bonds, chain_leaving_atom_indices = self._get_inter_and_intra_residue_bonds(atom_array, chain_id, chain_info_dict[chain_id]['type'])
    ...
    def _load_monomer_sequence_information(self, cif_block, chain_info_dict, atom_array):
        ...
        polymer_seq_df = polymer_seq_df.loc[:, ['entity_id', 'num', 'mon_id']].rename(columns={'num': 'residue_id', 'mon_id': 'residue_name'})
        ...
        unique_residues = {residue for residues in polymer_entity_id_to_residue_names_and_ids.values() for residue in residues['residue_names']}
        for chain_id in deduplicate_iterator(struc.get_chains(atom_array)):
            ...
            if entity_id in polymer_entity_id_to_residue_names_and_ids:
                ...
                if polymer_entity_id_to_residue_names_and_ids[entity_id]['residue_names']:
                    chain_info_dict[chain_id]['residue_name_list'] = polymer_entity_id_to_residue_names_and_ids[entity_id]['residue_names']
                    chain_info_dict[chain_id]['residue_id_list'] = polymer_entity_id_to_residue_names_and_ids[entity_id]['residue_ids']
            else:
        ...
        chain_info_dict = {chain_id: chain_info for chain_id, chain_info in chain_info_dict.items() if 'residue_name_list' in chain_info}
