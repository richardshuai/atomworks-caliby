...
from cifutils.pdbx.reader.PdbxContainers import DataCategory

import cifutils.obutils as obutils
import cifutils.geometry as geometry
        ...
        if pdbx_poly_seq_scheme is not None:
            ...
            for entity_id,res in itertools.groupby(residues, key=lambda x : x[0]):
                ...
                for k, v in itertools.groupby(res, key=lambda x: x[0]):  # Key is the sequence number, stored in x[0]
                    ...
                    *alternative_residues, last_residue = v_list # Split list of residues into the first residue and the rest of the residues
