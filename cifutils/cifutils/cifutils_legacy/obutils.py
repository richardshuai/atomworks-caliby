...
def FindAutomorphisms(obmol : openbabel.OBMol,
                      heavy : bool = True,
                      maxmem : int = 20 * (2**30),
                      max_automorphs = 10 ** 6) -> torch.tensor:
    ...
    try:
        openbabel.FindAutomorphisms(obmol,automorphs,mask,maxmem) # Returns a vector of vectors of pairs of unsigned ints (atom indices)
        if automorphs.size() > max_automorphs: # We can't process too many automorphisms
            logging.warning(f"Large number of automorphisms: {automorphs.size()}. Subsampling to {max_automorphs} automorphisms.")
