...
4. (Within `load_from_row`) Parse the dataframe row into a standardized format we can proceed with. Although often trivial, we must standardize the outputs of individual dataset `__getitem__` before proceeding, build the path to the CIF/PDB file, and aggregate the appropriate information. For our example, we may use the `PNUnitsDFParser` (which inherits the `ABC` `RowParser`) to parse our `pd.Series` row into a dictionary like `{"example_id": "{pn_unit}{3ne2}{2}{A_1}", "path": "/somewhere/on/digs/3ne2.cif", "q_pn_units": "A_1"}`. Only the `example_id` and the `path` are required; however, some transformations must receive additional fiels (`q_pn_units` for cropping, for example).
...
7. Last, with our complete dictionary containing the output of the `CIFParser` and possibly additional fields from the `RowParser` (optional, depending on the `Transforms` used), execute the `Transforms` pipeline, and return the featurized results.
...
### Step 2: Write a `RowParser` for the dataframe.
As described in `datahub/datasets/dataframe_parsers`, we need `RowParsers` to convert the information held in a loaded row into a common format.
