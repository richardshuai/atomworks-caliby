...
class AtomizeResidues(Transform):
    ...
    def validate_atomize(self, atom_array: AtomArray, atomize: np.ndarray):
    ...
    def forward(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        atomize = np.full(len(atom_array), self.atomize_by_default, dtype=bool)

        # Exclude residues to ignore
        atomize[np.isin(atom_array.res_name, self.res_names_to_ignore)] = False

        # Include residues to atomize
        atomize[np.isin(atom_array.res_name, self.res_names_to_atomize)] = True
        ...
        if "atomize" in atom_array.get_annotation_categories():
            ...
            self.validate_atomize(atom_array, atomize)

        # Perform atomization
        atom_array.set_annotation("atomize", atomize)

        if self.move_atomized_part_to_end:
