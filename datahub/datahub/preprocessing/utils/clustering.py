...
from datahub.clustering.fasta import create_fasta_file
...
def run_mmseqs2_clustering(
    ...
    """
    temp_dir = Path.cwd() / "temp" if not exists(temp_dir) else Path(temp_dir)
    try:
...
def cluster_all_sequences(
    ...
    create_fasta_file(df, sequence_col_name, mmseqs_fasta_path)
