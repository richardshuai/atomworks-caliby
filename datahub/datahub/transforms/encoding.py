...
def atom_array_to_encoding(
    ...
    use_alt_atom_name_if_std_name_fails: bool = True,
    ...
    Args:
        ...
        - use_alt_atom_name_if_std_name_fails (bool, optional): If True, attempts to match the atom name
          to an alternative name if the standard atom name is not found in the encoding. Defaults to True.
          This is primarily for backward compatibility with older encodings (e.g., RF2AA_ATOM36_ENCODING)
          that used alternative atom names for hydrogens.
    ...
    for i, token in enumerate(token_iter(atom_array)):
        ...
        if (has_atomize and token.atomize[0]) or len(token) == 1:
            token_name = int(token.element[0])
            token_is_atom = True
        ...
        for atom in token:
            ...
            elif use_alt_atom_name_if_std_name_fails and not token_is_atom:
                alt_to_std = get_std_alt_atom_id_conversion(token_name)["alt_to_std"]
            ...
            # ... case 4: failed to find the relevant atom_name for this token when we should, so we raise an error
                ...
                if use_alt_atom_name_if_std_name_fails:
                    msg += f" and neither is the alternative atom name ({token_name}, {alt_atom_name})."
                msg += "\nProblematic atom:\n"
                msg += f"{atom}"
...
class EncodeAtomArray(Transform):
    ...
    def __init__(
        ...
        Args:
            ...
            - `use_alt_atom_name_if_std_name_fails` (bool, optional): If True, try to match the atom name to an
                alternative name if the standard atom name is not in the encoding. Defaults to True.
                For example, the old `RF2AA_ATOM36_ENCODING` uses alternative atom names for the hydrogens (e.g.
                "2H" instead of "H2" in `MET`.) This is functionality is mostly for backwards compatibility
        ...
        self.use_alt_atom_name_if_std_name_fails = use_alt_atom_name_if_std_name_fails
    ...
    def forward(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        encoded = atom_array_to_encoding(
            ...
            use_alt_atom_name_if_std_name_fails=self.use_alt_atom_name_if_std_name_fails,
...
class AddTokenAnnotation(Transform):
    ...
    def forward(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for i, token in enumerate(token_iter(atom_array)):
            # ... extract token name
            if (has_atomize and token.atomize[0]) or len(token) == 1:
                assert len(token.element) == 1, "Atomize annotation is only allowed for single atoms."
                token_name = token.element[0]
                token_is_atom = True
            else:
                token_name = token.res_name[0]
                token_is_atom = False
