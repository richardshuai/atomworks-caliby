...
def process_pdb_id_group(group: pd.DataFrame) -> list[dict]:
    ...
    close_pn_units_dict = {
        row["q_pn_unit_iid"]: json.loads(row["q_pn_unit_close_pn_unit_iids"])
        if row["q_pn_unit_close_pn_unit_iids"]
        else []
        for _, row in group.iterrows()
    }
    ...
    for pn_unit_1, pn_unit_2 in combinations(pn_units, 2):
        if (
            ...
            pn_unit_1_data = group[group["q_pn_unit_iid"] == pn_unit_1].iloc[0]
            pn_unit_2_data = group[group["q_pn_unit_iid"] == pn_unit_2].iloc[0]
            ...
            interface = {
                ...
                "close_pn_unit_iids": json.dumps(
                    list(set(close_pn_units_dict[sorted_pair[0]] + close_pn_units_dict[sorted_pair[1]]))
                ),
            ...
            # Dynamic columns from clustering
