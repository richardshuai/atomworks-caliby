...
def get_pn_units_with_non_biological_bonds(atom_array, bond_mask) -> np.ndarray:
    ...
    atom_a_elements = atom_array.element[bonds_to_check[:, 0]].astype(int)
    atom_b_elements = atom_array.element[bonds_to_check[:, 1]].astype(int)
...
def handle_clashing_pn_units(clashing_pn_units_set, clashing_pn_units_dict, atom_array, pn_unit_iid_map) -> AtomArray:
    ...
    for pn_unit in clashing_pn_units_set:
        ...
        clashing_pn_unit_details[pn_unit] = {
            ...
            "is_metal": pn_unit_atom_array[0].is_metal,
...
def get_bonded_polymer_pn_units(query_pn_unit_iid, filtered_atom_array) -> set[str]:
    ...
    inter_pn_unit_bonds = filtered_atom_array.bonds.as_array()[
        data.data_preprocessor.DataPreprocessor.get_inter_pn_unit_bond_mask(filtered_atom_array)
    ]
    ...
    polymer_pn_unit_iids = set(filtered_atom_array.pn_unit_iid[filtered_atom_array.chain_type != ChainType.NON_POLYMER])
